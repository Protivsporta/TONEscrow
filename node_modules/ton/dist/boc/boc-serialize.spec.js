...
describe('boc-serialize', () => {
    ...
    it('should serialize boc with many refs', async () => {
        const data = await (0, promises_1.readFile)(path_1.default.resolve(__dirname, '__testdata__', 'manyCells.txt'), { encoding: 'utf-8' });
        const dataBuffer = Buffer.from(data, 'base64');
    ...
    it('should serialize and parse exotics', async () => {
        const cell = new Cell_1.Cell('pruned');
        cell.refs.push(new Cell_1.Cell('merkle_proof'));
        cell.refs.push(new Cell_1.Cell('merkle_update'));
        let res = cell.toBoc({ idx: false });
        let p = Cell_1.Cell.fromBoc(res);
    });
});
